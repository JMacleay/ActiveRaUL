...
function getExtData(dataURI){
	...
	var requestURI='http://'+document.location.hostname;
	if(document.location.port!=='')
		requestURI+=':'+document.location.port;
	requestURI+='/raul/service/util/extraInfo?url='+dataURI;
	jQuery.ajax({
			type: 'GET',
			async: false,
			url: requestURI,
...
function parseData(data, main_subjectURI){
	...
	var groups={};
	jQuery('[typeof="raul:Group"]').each(function(index,el){
		var groupAbout=jQuery(el).attr('about');
		var groupValue=jQuery(el).find("span[property=raul:value]").first().text();
		var groupPredValue=jQuery('[about="'+groupValue+'"]').children("[property='rdf:predicate']").text();
		if(groups[groupPredValue]==null||groups[groupPredValue]==undefined){
			//var groupPredValue=jQuery('[about="'+groupValue+'"]').children("[property='rdf:predicate']").text();
			groups[groupPredValue]=groupAbout;
		}
	});
	
	jQuery.each(groups,function(index,val){
		var li=jQuery.find('li[resource="'+val+'"]').first();
		if(li==undefined||li==null)
			return;
		var listName=jQuery(li).parent().attr('about');
		jQuery('div[typeof="raul:DynamicGroup"]').each(function(index1,el){
			var listsLength=jQuery(el).find('span[property="raul:list"]').length;
			jQuery(el).find('span[property="raul:list"]').each(function(index11,el1){
				if(jQuery(el1).text()==listName){
					var nodesLength=jQuery(xmlDocument).find("[nodeName=" + index +"]").length;
					if(listsLength<nodesLength){
						var dynamicGroupAbout=jQuery(el).attr('about');
						var buttonId=jQuery('input[group="'+dynamicGroupAbout+'"]').attr('id');
						for(var i=0;i<nodesLength-listsLength;i++){
							dynamicWidgetsAdd(dynamicGroupAbout, buttonId);
						}
					}
					if(listsLength>nodesLength){
						var dynamicGroupAbout=jQuery(el).attr('about');
						var button=jQuery('input[group="'+dynamicGroupAbout+'"]');
						var i=1;
						jQuery(el).find('span[property="raul:list"]').each(function(index12,el2){
							if(i>nodesLength){
								dynamicWidgetsRemove(jQuery(el2).text(),dynamicGroupAbout,'','');
							}
							i++;
						});
					}
