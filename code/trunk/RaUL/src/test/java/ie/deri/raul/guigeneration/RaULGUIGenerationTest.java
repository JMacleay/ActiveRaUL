...
//		}
		...
		for (Entry<String, LinkedHashMap<String, List<String>>> entry : uri.entrySet()) {
			...
			output = "Superclass:  " + entry.getKey();
			//bw.write(output);
			//bw.write("\n");
			...
			for (Entry<String, List<String>> subClassEntry : predicatevalue.entrySet()) {
				...
				output = "  Subclass:  " + subClassEntry.getKey();
				//bw.write(output);
				//bw.write("\n");
		...
		for (Entry<String, LinkedHashMap<String, List<String>>> entry : uri.entrySet()) {
			...
			output = "\n" +
					"<rdf:Description rdf:about=\""+ instanceURI +"#" + fragmentID + "\">\n" + //Superclass instance
					"  <rdf:type rdf:resource=\"" + entry.getKey() + "\"></rdf:type>";
			bw.write(output);
			...
			for (Entry<String, List<String>> subClassEntry : subclassvalue.entrySet()) {
				...
				output =  "  <rdf:_" + i + " rdf:resource=\""+ instanceURI + "#" + fragmentID +"_container\"/>";
				bw.write(output);
			...
			for (Entry<String, List<String>> subClassEntry : subclassvalue.entrySet()) {
				...
				"</rdf:Description>");
				output =  "<rdf:Description rdf:about=\""+ instanceURI + "#" + fragmentID +"_container\"/>\n" +			
						"  <rdf:type rdf:resource=\"http://purl.org/NET/raul#WidgetContainer\"></rdf:type>\n" +
						"  <raul:list>" + instanceURI + "#" + fragmentID +"</raul:list>\n" +
				"</rdf:Description>";
				bw.write(output);
				...
				output =  "<rdf:Description rdf:about=\"" + instanceURI + "#" + fragmentID +"\">\n" +
						"  <rdf:type rdf:resource=\"" + subClassEntry.getKey() + "\"></rdf:type>";
				bw.write(output);
				...
				while (iterator.hasNext()) {
				    ...
				    output =  "  <rdf:_" + j + " rdf:resource=\"" + instanceURI + "#" + elements[4].substring(elements[4].lastIndexOf("/")+1) + "\"/>";
					bw.write(output);
				...
				while (iterator.hasNext()) {
					...
					if(!seenPredicates.containsKey(elements[4])) {
						...
						output =  "<raul:Textbox rdf:about=\"" + instanceURI + "#" + elements[4].substring(elements[4].lastIndexOf("/")+1) + ">\n" +
								"  <raul:label>" + elements[3] +"</raul:label>\n" +
								"  <raul:value>" + instanceURI + "#value_" + elements[4].substring(elements[4].lastIndexOf("/")+1) + "</raul:value>\n" + 
								" </raul:Textbox>\n" +
								"<rdf:Description rdf:about="+ instanceURI + "#value_" + elements[4].substring(elements[4].lastIndexOf("/")+1) + ">\n" +
