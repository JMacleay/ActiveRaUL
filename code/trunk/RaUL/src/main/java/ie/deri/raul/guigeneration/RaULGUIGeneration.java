...
public class RaULGUIGeneration {
	...
	public LinkedHashMap<String, LinkedHashMap<String, List<String>>> generateGUI(Repository graph) throws RepositoryConfigException, RepositoryException, QueryEvaluationException {
	RepositoryConnection con = graph.getConnection();
		...
		if(ontologyNameSet.hasNext()) {
			...
			try {
						/*	query = con.prepareTupleQuery(QueryLanguage.SPARQL, 
									...
									"PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#>" +
									"PREFIX owl:<http://www.w3.org/2002/07/owl#>" +
									"SELECT ?cl WHERE {?cl rdf:type rdfs:Class." +
									" OPTIONAL {?cl rdfs:subClassOf ?sc. " +
									"filter(?cl != ?sc && ?sc != owl:Thing && ?sc != rdfs:Resource && ?sc!= rdfs:Class)} " +
				...
				query = con.prepareTupleQuery(QueryLanguage.SERQL, "SELECT DISTINCT s " +
						"FROM {s} rdf:type {y} " +
						"WHERE y " +
						"IN (rdfs:Class, owl:Class) " +
						"AND NOT s LIKE \"http://www.w3.org/2000/01/rdf-schema*\" " +
						"AND NOT s LIKE \"http://www.w3.org/2002/07/owl#*\" " +
						"AND NOT s LIKE \"http://www.w3.org/1999/02/22-rdf-syntax-ns*\" " +
						"AND NOT s LIKE \"http://www.w3.org/2001/XMLSchema#*\" " +
						"MINUS SELECT DISTINCT s " +
						"FROM {s} rdfs:subClassOf {y} " +
						"WHERE s != y " +
						"AND y != owl:Thing " +
						"AND y != rdfs:Resource " +
						"AND y != rdfs:Class " +
			...
			while (superClassResult.hasNext()) {
				...
				System.out.println(" * * * * * * * * * *  * * * * * *  **  * * * ");
				System.out.println("Supper Class :" + subjectString);
				//System.out.println(" * * * * * * * * * *  * * * * * *  **  * * * ");
				...
				try {
					...
					query = con.prepareTupleQuery(QueryLanguage.SERQL, "" +
							"SELECT s " +
							"FROM {s} rdfs:domain {y} " +
							"WHERE y IN (rdfs:Resource, owl:Thing) " +
							"AND NOT s LIKE \"http://www.w3.org/2000/01/rdf-schema*\" " +
							"AND NOT s LIKE \"http://www.w3.org/2002/07/owl#*\" " +
							"AND NOT s LIKE \"http://www.w3.org/1999/02/22-rdf-syntax-ns*\" " +
							"USING NAMESPACE rdf = <http://www.w3.org/1999/02/22-rdf-syntax-ns#>, " +
				...
				while (topPredicates.hasNext()) {
					...
					System.out.println(" * * * * * * * * * *  * * * * * *  **  * * * ");
					System.out.println("Top Predicate  :" + predicateDomainSet);
					try {
						
						// Query for getting label of a predicate
						
						query = con.prepareTupleQuery(QueryLanguage.SERQL, "SELECT o " +
								"FROM {s} rdfs:label {o} " +
								"WHERE s = <" + predicateDomainSet.getValue("s") + "> " +
								"USING NAMESPACE rdf = <http://www.w3.org/1999/02/22-rdf-syntax-ns#>, " +
								"owl = <http://www.w3.org/2002/07/owl#>, " +
								"rdfs = <http://www.w3.org/2000/01/rdf-schema#>");
					} catch (MalformedQueryException e1) {
					...
					if (predicateLabel.hasNext()) {
						...
						System.out.println("Top Predicate Label :" + labelString );						
					}
					
					
					try {
						//Query to know the type of the predicate to distinguish DataType from ObjectType properties .
					...
					while (predicateType.hasNext()) {
						...
						//System.out.println("  ");
						System.out.println("Top Predicate Data Type :" + predicateTypeSet);
						...
						if (predicateTypeSet.getValue("o").stringValue().equals("http://www.w3.org/2002/07/owl#DatatypeProperty")) {
							...
							while (dataType.hasNext()) {
								...
								System.out.println("Top Predicate Data Type Type :" + dataTypes);
							}	
							// Add a string to property list that contains comma separated values of the propertyType, range type, label, propertyURI
						...
						if (predicateTypeSet.getValue("o").stringValue().equals("http://www.w3.org/2002/07/owl#ObjectProperty")) { 
							...
							try {
								query = con.prepareTupleQuery(QueryLanguage.SPARQL, "" +
										...
										"rdfs:<http://www.w3.org/2000/01/rdf-schema#>\n" + 
										"SELECT ?o " +
										"WHERE {<" + predicateDomainSet.getValue("s") + "> rdfs:range ?o.}");
							} catch (MalformedQueryException e1) {
								e1.printStackTrace();
							}
							...
							while (propertyRange.hasNext()) {
								...
								System.out.println("Top Predicate Data Type Range :" + propertyRangeSet.getValue("o"));
								// Add a string to property list that contains comma separated values of the propertyType, range type, label, propertyURI
	...
	private LinkedHashMap<String, List<String>> getSubClassPropertyList(RepositoryConnection con, String topClass, String superClass, List<String> propertyList) throws RepositoryException, QueryEvaluationException {
