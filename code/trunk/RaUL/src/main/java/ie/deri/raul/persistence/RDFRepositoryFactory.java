...
public abstract class RDFRepositoryFactory {
	...
	/**
	 * Creates a {@link RDFRepository} by with the repository name specified in
	 * the raul.properties file.
	 * 
	...
	public static RDFRepository createRepository() throws RepositoryException {
		...
		if (repository == null || "".equals(repository)) {
			throw new RepositoryException(
					"Property 'default.repository' undefined.");
		}
		return createRepository(repository);
	}

	public static RDFRepository createRepository(String repositoryId)
			throws RepositoryException {
		...
		if (repoType != null && "remote".equalsIgnoreCase(repoType)) {
			...
			if (url == null || "".equals(url)) {
				throw new RepositoryException(
						"Please specify remote repository location in property 'repository.url'.");
			}
			...
			// we assume we always want inferencing - TODO put inferencing
			...
			try {
				...
				throw new RepositoryException(
						"Error while creating an in-memory SailRepository.");
			}
		}else if(repoType != null && "native".equalsIgnoreCase(repoType)) {
			// we assume we always want inferencing - TODO put inferencing
			// boolean flag in config
			boolean inferencing = true;
			
			 // Unlock sesame dir if the server didn't cleanly shutdown
			//File lock = new File("/home/u5096831/sesame/lock");
			//if(lock.exists()) lock.delete();
	        
			File dataDir = new File("/home/u5096831/sesame");
	        ...
	        try {
				...
				throw new RepositoryException(
						e + "Error while creating an native SailRepository.");
				
			} finally{
				//com.sun.org.apache.bcel.internal.Repository.clearCache();
			}
		}

		// default action if no valid property found
		throw new RepositoryException(
				"No valid property 'repository.type'. Please specify either 'remote' or 'inmemory'.");
	}

	public static RDFRepository createInMemoryRepository()
			throws RepositoryException {
		boolean inferencing = true;
		try {
			Repository repository = null;
