...
public class GenerateRaULRDFTest {
	...
	public GenerateRaULRDFTest(RDFRepository repository, String endpoint, String classURI) {
	...
	public GenerateRaULRDFTest(Repository myRepository, String endpoint, String classURI) {
		// TODO Auto-generated constructor stub
		try{ 
			...
			System.out.println(generateRaULBasedRDFGraph());
		//	System.out.println(getInstances(classURI));
			//System.out.println(getSuperClasses(this.classURI));
		//	System.out.println(getRelevantPropertiesListTest(this.classURI));
		}catch(Exception e){}
	}
	...
	public String writePageProperty(){
		...
		_output = "<rdf:Description rdf:about=\""+endpoint+"\">\n" +
				...
				"	<rdfs:range>"+classURI+"</rdfs:range>\n" +
				"</rdf:Description>\n";
	...
	public String writePropertiesDescription(String _classURI, String propertyURI, String uri, int dropCount, int level) throws Exception{
		...
		String labelTag ="";
		if(_classURI.contains("#")){
			labelTag = _classURI.split("#")[1];}
		else {
			labelTag = _classURI.substring(_classURI.lastIndexOf("/")+1);
		}
			
		output = output + writeTextBoxProperty(_resourceURI, labelTag + " Name", _property, "", false , false, "DatatypeProperty", _classURI );
	    allURIs.add(_resourceURI);
	    
		if(classURI.equals(_classURI)){
			...
			output= output + writeListBoxProperty(_resourceURI+"_type", "","http://www.w3.org/1999/02/22-rdf-syntax-ns#types", _subClasses);
		...
		_logger.info(" Class properties size : " + al.size());
	//	int dropCount = 0;
			...
			if (range.equalsIgnoreCase("") && datatype.equalsIgnoreCase("ObjectProperty")){
			...
			} else if (datatype.equalsIgnoreCase("DatatypeProperty")){
				 //System.out.println( property + " is a datatype Property"  + datatype);
				 String rangeClass="";
				 String comment = "DatatypeProperty";
			...
			} else if (getPropertiesCount(range) < 1 ){
				String comment="";
				String rangeClass="";
				...
				if(isFunctional(property)){
					System.out.println( property + " is a functional Property with no further property" );
					HashMap<String, String> instances = new HashMap<String, String>();
					instances = getInstances(range);
				//	System.out.println( "INSTANCESSSSS " + instances);
					if(instances.size()<1){						
					 output = output + writeTextBoxProperty(resourceURI, label, property, "", false , false, comment, rangeClass );
					 } else {
						 output = output + writeListBoxProperty(resourceURI, label ,property, instances);
					 }
					 allURIs.add(resourceURI);
				} else {
					System.out.println( property + " is a nonFunctional Property" );
				...
				System.out.println("alreadyTraversed" + alreadyTraversed);
				...
				for (Entry<String,String> entry : subClasses.entrySet()){
					System.out.println("SubClass of  " + range);
					if(traversedNodes.containsKey(entry.getKey())){
						alreadyTraversed = true;	
						System.out.println("Traverse Node contains " );
						break;
					} else {}
				}
				...
				if (isTransitive(property)){
					...
					if(isFunctional(property)){
						...
						if(subClasses.size() > 1){
							...
							output= output + writeListBoxProperty(resourceURI+"_type", "","http://www.w3.org/1999/02/22-rdf-syntax-ns#types", subClasses);
							allURIs.add(resourceURI);
	...
	public String writeListBoxProperty(String resourceURI,String label, String property,HashMap<String,String> listMembers) throws RepositoryException{
		...
		_output = _output + "	<raul:label>"+ label +"</raul:label>\n" ;
		}
		_output = _output +	"	<raul:name>"+resourceURI.split("#")[1]+"</raul:name>\n" +
				"	<raul:list>"+listURI+"</raul:list>\n" +
				"	<raul:value>"+valueURI+"</raul:value>\n" +
	...
	public String writeListMembers(String resourceURI, String subClassURI, String label){
		...
		//System.out.println(subClassURI +	"	LABEL:	"	+ label );
...
//    	predicate = subClassURI.substring(endOfPrefix+1);
		...
		String output = "<rdf:Description rdf:about=\""+resourceURI+"\">\n"+
			...
			"	<raul:value>"+subClassURI+"</raul:value>\n" +
			"</rdf:Description>\n";
...
//    	
		output = output + "<rdf:Description rdf:about=\""+valueURI+"\">\n"+
				...
				"	<rdf:predicate>"+property+"</rdf:predicate>\n"+
			...
			String queryString = "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> " +
					...
					"WHERE {" +
						"{"+ 
							...
							"?restriction ?rProperty ?range. " +
							"FILTER (" +
								"(?rProperty = owl:allValuesFrom) || " +
								"(?rProperty = owl:someValuesFrom) || " +
								"(?rProperty = owl:minCardinality) || " +
								"(?rProperty = owl:maxCardinality) " +
								")" +
//								"{" +
...
//									"UNION " +
//									"{?restriction owl:minCardinality ?range} " +
//									"UNION " +
//									"{?restriction owl:maxCardinality ?range} " +
//								"}" +
							"}" +
							" UNION " +
							"{" +
							...
							"OPTIONAL {" +
								"?restriction owl:someValuesFrom ?range.} " +
							//"OPTIONAL {?restriction owl:minCardinality ?range.} "+
							"}" +
							" UNION " +
							"{" +
							"<"+URI+"> rdfs:subClassOf ?equClass." +
							"?equClass owl:intersectionOf ?collection." +
							"?collection ?collectionProperty ?member." +
							"?member ?subClassOf ?restriction." +
							"?restriction owl:onProperty ?property." +
							...
							" UNION " +
							"{ <"+URI+"> owl:equivalentClass ?equClass." +
							"?equClass owl:onProperty ?property." +
							"?equClass owl:someValuesFrom ?collection." +
							"?collection owl:intersectionOf ?list." +
							"?list rdf:first ?range." +
							"}" +
...
//				type = "ObjectProperty";
	...
	public HashMap<String,String> getSubClasses(String URI) throws RepositoryException, QueryEvaluationException,  MalformedQueryException{
		...
		while(resultSet.hasNext()) {
			BindingSet results = resultSet.next();
			String className = results.getValue("subClass").toString();
			String label = "";
					if (results.hasBinding("label")) {
			...
			subClassList.put(className, label);
			//System.out.println("SubClass   " + results.getValue("subClass").toString() + "SubClass Label  " + results.getValue("label").stringValue());
		}	
		//System.out.println( " GET SUBCLASS FUNCTION " + subClassList);
		return subClassList;
	}
	
	public HashMap<String,String> getInstances(String URI) throws RepositoryException, QueryEvaluationException,  MalformedQueryException{
		HashMap<String,String> subClassList = new HashMap<String,String>();
		//HashMap<String, String> properties = new HashMap<String, String>();
		int count= 0;
		try {	
		...
		System.out.println( " GET INSTANCE FUNCTION " + subClassList);
		return subClassList;
	}
